...
class EmotionVM: ObservableObject {
    
    // goal is to store the emotion data with core data
    
    @AppStorage("NumberBreaths") private var NumberBreaths: Int = 0
    
    // array to hold emotion data
    @Published var emotions: [EmotionM] = []
    
    lazy var persistentContainer: NSPersistentContainer = {
        let container = NSPersistentContainer(name: "EmotionCoreData")
    ...
    func saveEmotion(emotion: EmotionM) {
        print(emotion)
        ...
        // create a new emotion with the viewContext that contains emotion data from core data
           let emotionEntry = EmotionObject(context: viewContext)
    ...
    // Computed property to read the number of breaths
        var breathCount: Int {
            NumberBreaths
        }
